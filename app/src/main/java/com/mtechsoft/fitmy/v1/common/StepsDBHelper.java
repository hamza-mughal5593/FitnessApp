...
public class StepsDBHelper extends SQLiteOpenHelper {
    ...
    public ArrayList<DateStepsModel> readStepsEntriesToday() {
        ArrayList mStepCountList = new ArrayList();

        Calendar mCalendar = Calendar.getInstance();
        String todayDate = String.valueOf(mCalendar.get(Calendar.MONTH)) + "/" + String.valueOf(mCalendar.get(Calendar.DAY_OF_MONTH)) + "/" + String.valueOf(mCalendar.get(Calendar.YEAR));
        String selectQuery = "SELECT * FROM " + TABLE_STEPS_SUMMARY + " WHERE " + CREATION_DATE + " ='" + todayDate + "'";
        try {
    ...
    public ArrayList<DateStepsModel> readStepsEntriesWeek() {
        ArrayList mStepCountList = new ArrayList();

        Calendar mCalendar = Calendar.getInstance();
        mCalendar.set(Calendar.DAY_OF_WEEK, Calendar.SUNDAY);
        String startDate = String.valueOf(mCalendar.get(Calendar.MONTH)) + "/" + String.valueOf(mCalendar.get(Calendar.DAY_OF_MONTH)) + "/" + String.valueOf(mCalendar.get(Calendar.YEAR));

        mCalendar.add(Calendar.DAY_OF_MONTH, 6);
        String endDate = String.valueOf(mCalendar.get(Calendar.MONTH)) + "/" + String.valueOf(mCalendar.get(Calendar.DAY_OF_MONTH)) + "/" + String.valueOf(mCalendar.get(Calendar.YEAR));


        String selectQuery = String.format("SELECT * FROM StepsSummary WHERE creationdate BETWEEN '%s' and '%s'", startDate, endDate);
        try {
            SQLiteDatabase db = this.getReadableDatabase();
            Cursor c = db.rawQuery(selectQuery, null);
            if (c.moveToFirst()) {
